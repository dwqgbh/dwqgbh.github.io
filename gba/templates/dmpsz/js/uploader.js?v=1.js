$(function () {
    ...
    $.upLoadDefaults.property = {
        ...
        isshowimg: false, //是否显示预览图
        water: false, //是否加水印
        thumbnail: false, //是否生成缩略图
        sendurl: null, //发送地址
        filetypes: "gif,jpg,jpeg,png,heif,heic", //文件类型
        filesize: "2048", //文件大小
        btntext: "", //上传按钮的文字
        ...
        var fun = function (parentObj) {
            ...
            uploader.on('uploadProgress', function (file, percentage) {
                ...
                //progressObj.children(".txt").html(file.name);
				 progressObj.children(".txt").html('正在上传，请稍候...');
            ...
            uploader.on('uploadSuccess', function (file, data) {
                ...
                if (data.status == '1') {
                    ...
                    if (!p.multiple) {
                        ...
                        if (data.path.length > 0 && p.isshowimg) {//说明图片上传成功，名称正常
                            var progressObjimg = parentObj.siblings(".imgshow");
                            progressObjimg.html("<div class='fd' file-data='" + data.path + "?x-oss-process=image/format,jpg'><img src='" + data.path + "?x-oss-process=image/format,jpg'/></div>");
$(document).ready(function(){
   $(".imgshow .fd").click(function () {
        var img = $(this).attr("file-data");
        $(".fdpicshowtcc .pic .url").html("<img src='" + img + "?x-oss-process=image/format,jpg'/><span></span>");
		$(".fdpicshowtcc").addClass("show");
    });
   $(".fdpicshowtcc").click(function () {
        $(".fdpicshowtcc .pic .url").html("");
        $(".fdpicshowtcc").removeClass("show");
    });
});
                        }
                    } else {
...
function addImage(targetObj, originalSrc, thumbSrc) {
    ...
    var hidspecidhtml = "";
    if ($("#hid_photo_spec_id_curren")) {
        var hid_photo_spec_id_currenval = $("#hid_photo_spec_id_curren").val();
        if (hid_photo_spec_id_currenval) {
            hidspecidhtml = '<input type="hidden" name="hid_photo_spec_id" value="' + hid_photo_spec_id_currenval + '" />'
        }
    }
    
    //插入到相册UL里面
    var newLi = $('<li class="lispec_' + hid_photo_spec_id_currenval + '">'
    ...
    + '<input type="text" class="photo_sort_id" name="txt_photo_sort_id" value="99" />'
    + '</li>');
    ...
    $(".photo_sort_id").blur(function () {
        $(this).parent().find("input[name='hid_photo_sort_id']").val($(this).val());
    });

}
//设置相册封面
function setFocusImg(obj) {
    var ishavespecalbum=false;
    var hidspecidhtml = "";
    if ($("#hid_photo_spec_id_curren") && $("#hid_photo_spec_id_curren").val()) {
         ishavespecalbum = true;
    }

    if (ishavespecalbum) {
        $("#divspeccontent .img-box").removeClass("selected");
        $(".focus-photo").eq(0).val($(obj).children("img").eq(0).attr("src"));
